...
import deleteChat from "@/actions/deleteChat.action";
import handleToggle from "@/utils/haptics";
import timeAgo from "@/utils/timeago";
const SideBar = memo(
  ({ setShowBar, chatlist, addChatlist, removeChatList }) => {
    ...
    return (
      <div className="flex gap-2">
        <div className="w-full bg-linear-to-r bg-black p-2 h-screen flex flex-col">
          ...
          <div className="mb-4 border-dotted w-full ">
            ...
            <div class="flex items-center gap-2 mt-4 sm:gap-4">
              <Link
                href="/about-orion"
                onClick={handleToggle}
                className="text-[10px] w-full text-center tracking-[.25em] text-blue-400 font-bold uppercase border border-blue-500/30 px-3 py-2 rounded bg-blue-500/10"
              >
                About Orion
              </Link>
              <Link
          ...
          <div className="text-xl my-4 flex justify-between font-bold">
            Recent chats
            {chatlist.length > 0 && (
              <button
                className="text-sm cursor-pointer flex items-center bg-red-500/20 border border-red-900 rounded-lg px-2 gap-0.5"
                onClick={() => {
                  setShowDelete(!showDelete);
                  handleToggle();
                }}
              >
                <MessageSquareOff className="scale-70" />
                Delete
              </button>
            )}
          </div>
              ...
              chatlist?.map((e) => (
                <div key={e._id} className="flex gap-2 items-center ">
                  <div
                    ...
                    className="bg-gray-700/30 w-full  justify-between mb-2 p-2 hover:text-purple-500  cursor-pointer hover:scale-97 duration-300  hover:border-gray-700 border border-gray-500/40 py-4 flex items-center gap-2 rounded-xl"
                  >
                  ...
                  <button
                    ...
                    className={`${showDelete ? "flex" : "hidden"} cursor-pointer active:scale-80 hover:scale-115 duration-300 bg-red-900/40 border rounded-lg border-red-700/30 p-2`}
                  >
